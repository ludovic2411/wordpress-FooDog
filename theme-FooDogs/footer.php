<?php

 ?>
<!DOCTYPE html>
<html lang="en" dir="ltr">
  <head>
    <link rel="stylesheet" href="style.css">
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.0.8/css/all.css" integrity="sha384-3AB7yXWz4OeoZcPbieVW64vVXEwADiYyAEhwilzWsLw+9FgqpyjjStpPnpBO8o8S" crossorigin="anonymous">
    <link href="https://fonts.googleapis.com/css?family=Crimson+Text" rel="stylesheet">
    <meta charset="utf-8">
    <title></title>
  </head>
  <body>
    <!-- retirer ce qui suit au dessus -->
    <!--le container reprend tout le footer -->
    <section class="footer-container">
      <!-- partie gris clair et première grille -->
      <div class="footer-navigation">
        <ul class="footer-categories">
          <li><h3></h3></li>
          <li><p><a href=""></a></p></li>
          <li><p><a href=""></a></p></li>
          <li><p><a href=""></a></p></li>
          <li><p><a href=""></a></p></li>
          <li><p><a href=""></a></p></li>
        </ul>
        <ul class="footer-popular-posts">
          <li><h3></h3></li>
          <li><p><a href="#"><img src="" alt=""></a></p></li>
          <li><p><a href="#"><img src="" alt=""></a></p></li>
          <li><p><a href="#"><img src="" alt=""></a></p></li>
        </ul>
        <!-- seconde grille -->
        <div>
        <ul class="instagram-title">
          <li><h3></h3></li>
        </ul>
        <ul class="instagram">
          <li><img class="left" class="item-1" src="item-1.jpg" alt="cute dog"></li>
          <li><img class="center" class="item-2" src="" alt="cute dog"></li>
          <li><img class="right" class="item-3" src="" alt="cute dog"></li>
          <li><img class="left" class="item-4" src="" alt="cute dog"></li>
          <li><img class="center" class="item-5" src="" alt="cute dog"></li>
          <li><img class="right" class="item-6" src="" alt="cute dog"></li>
          <li><img class="left" class="item-7" src="" alt="cute dog"></li>
          <li><img class="center" class="item-8" src="" alt="cute dog"></li>
          <li><img class="right" class="item-9" src="" alt="cute dog"></li>
        </ul>
      </div>
      </div>
      <!-- Partie gris foncé -->
      <div class="footer-social">
        <ul>
          <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
          <li><a href="#"><i class="fab fa-twitter"></i></a></li>
          <li><a href="#"><i class="fab fa-instagram"></i></a></li>
        </ul>
        <p class="copyright">2018 © Les Grosses Teubs</p>
      </div>
    </section>
    <?php wp_footer(); ?>
  </body>
</html>
